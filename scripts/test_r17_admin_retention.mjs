...
assert.match(ready.html,/Learner-impact failure trends/);
assert.match(ready.html,/writing_evaluator/);
assert.match(ready.html,/2 learner-impact failures/);
...
const impactInsufficient=await mount({ok:true,json:async()=>({available:true,has_data:true,window_days:7,total_activities:1,total_completions:0,skills:[],return_windows:[],daily_returning:[],learner_impact_failures:{available:true,data_state:'insufficient_data',has_data:false,by_capability:[]}})});
assert.match(impactInsufficient.html,/No validated learner-impact failures in this window/);
