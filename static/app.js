...
function syncTaskMode(){
  ...
  if(mode==='free'){
    ...
    $('#taskHint').textContent='Free writing: no prompt is required. Your language and clarity will still be evaluated.';
    return;
  }
  ...
  if(mode==='custom'){
    ...
    topic.disabled=true;
    length.disabled=true;
    $('#taskHint').textContent='Custom prompt: enter the exact task you want the evaluator to use for Task Achievement.';
    return;
  }
...
function renderGeneratedTask(d){
  ...
  $('#taskPreview').innerHTML=`
    ...
    <small>Target: about ${esc(d.word_target)} words · ${esc($('#target').value)}</small>`;
  $('#taskPreview').classList.remove('hidden');
}

async function generateTask(surprise=false){
  const modes=['opinion','email','review','story','toeic'];
  const topics=['daily life','work','technology','education','travel','environment','culture and media','shopping and services','communication','community'];
  ...
  try{
    ...
    $('#taskHint').textContent=d.source&&d.source.startsWith('ollama:')
      ?'Generated locally by Ollama. Generate again anytime for a new challenge.'
...
function newWriting(){
  ...
  $('#taskMode').value='free';
  $('#taskTopic').value='random';
  $('#taskLength').value='150';
  $('#taskPreview').innerHTML='';
  $('#taskPreview').classList.add('hidden');
  $('#writeTitle').textContent='Write';
  $('#writeSubtitle').textContent='Choose a practice task or write freely, then let the coach diagnose patterns.';
...
async function improveEssay(id,mode){
  ...
  try{
    ...
    const r=await fetch('/api/improve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:essay.text,target_cefr:essay.target_cefr||'B2',mode})});
    const d=await r.json();
