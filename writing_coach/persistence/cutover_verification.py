...
def compare_domain_parity(engine, discovery: Discovery) -> dict[str, Any]:
    ...
    with Session(engine) as s:
        pg_auth=sorted((u.user_key,u.email,u.name,u.role) for u in s.scalars(select(User)).all() if u.user_key != 'legacy')
        source_sub=sorted((str(r.get('user_key') or ''),str(r.get('plan_id') or 'free'),str(r.get('status') or 'active'),str(r.get('provider') or ''),str(r.get('external_customer_id') or ''),str(r.get('external_subscription_id') or '')) for r in _rows(discovery.product_db,'subscriptions'))
        target_sub=sorted((next((u.user_key for u in s.scalars(select(User).where(User.id==x.user_id)).all()),''),x.plan_id,x.status,x.provider,x.external_customer_id,x.external_subscription_id) for x in s.scalars(select(Subscription)).all())
        source_usage=sorted((str(r.get('user_key') or ''),str(r.get('feature') or ''),int(r.get('amount') or 0),str(r.get('request_id') or '')) for r in _rows(discovery.product_db,'usage_events'))
        users={u.id:u.user_key for u in s.scalars(select(User)).all()}
        target_usage=sorted((users[x.user_id],x.feature,x.amount,x.request_id) for x in s.scalars(select(UsageEvent)).all())
