...
import type {EvaluationInput, EvaluationResult, FreeTask, GenerateTaskInput, GrammarPractice} from '../api/contracts/learning';
...
export function useGenerateTask(client: ApiClient | null, sessionCookie?: string | null): UseMutationResult<FreeTask, ApiError, GenerateTaskInput> {
  return useMutation({mutationFn: (input) => client ? client.generateTask(input, {sessionCookie: sessionCookie ?? undefined}) : Promise.reject(new ApiError('configuration_missing', 'API client is unavailable')), retry: false});
}
