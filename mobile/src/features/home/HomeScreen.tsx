...
export function HomeScreen({client: providedClient}: {client?: ApiClient}) {
  ...
  if (session.status !== 'authenticated' || !sessionCookie || !client) return <Unavailable />;
  ...
  return <RecommendationCard recommendation={recommendation.data} starting={next.isPending} failed={next.isError} onStart={() => next.mutate(recommendation.data.target_level, {onSuccess: (task) => { setPracticeHandoff(task); router.push('/(app)/writing'); }})} />;
}

const styles = StyleSheet.create({container: {flex: 1, padding: 24, gap: 12}, title: {fontSize: 28, fontWeight: '700'}, body: {fontSize: 16, lineHeight: 24}, card: {padding: 16, borderRadius: 16, gap: 8}, cardTitle: {fontSize: 20, fontWeight: '700'}, button: {padding: 16, borderRadius: 12, alignItems: 'center', marginTop: 12}, buttonText: {color: '#fff', fontWeight: '700'}});
